...
namespace on_sales.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if(!await _roleManager.RoleExistsAsync(SD.ManagerRole))
                {
                    ...
                    _roleManager.CreateAsync(new IdentityRole(SD.ApproverRole)).GetAwaiter().GetResult();
                    _roleManager.CreateAsync(new IdentityRole(SD.CustomerRole)).GetAwaiter().GetResult();
                }
                
                if (result.Succeeded)
                {
                    if (role == SD.ApproverRole)
                    {
                        await _userManager.AddToRoleAsync(user, SD.ApproverRole);
                    }
                    else
                    {
